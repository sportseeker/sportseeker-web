...
$gallery_options = NextcodeGallery()->settings->getOptionsByView("justified");
$json = json_encode($gallery_options);

wp_enqueue_script("nextcodegalleryjustified", \NextcodeGallery()->pluginUrl() . "/resources/assets/js/frontend/ug-theme-justified.js", array('jquery'), false, true);
?>

<div id="nextcode_gallerycontainer_<?= $gallery_data->id_gallery ?>" style="display:none;" data-view="justified">
...
if ($gallery_data->display_type == 2) {
    ...
    <div class="nextcode_galleryload_more_space">
        <button data-id="<?= $gallery_data->id_gallery ?>" data-count="<?= $gallery_data->items_per_page ?>"
                class="nextcode_galleryload_more"><?= $options["load_more_text_justified"] ?>
